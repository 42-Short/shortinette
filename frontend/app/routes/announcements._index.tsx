...
export const meta: MetaFunction = () => {
  return [
    ...
    { name: "description", content: "What are we working on?" },
  ];
};
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const response = await fetch(
    `${process.env.API_BASE_URL}/admin/announcements/`,
    {
      method: "POST",
      headers: {
    ...
    return json(
      {
        errors: {
          message:
            "An internal server error occurred while creating the announcement. Please try again.",
        },
      },
      { status: 500 },
    );
